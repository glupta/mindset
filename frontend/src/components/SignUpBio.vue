<template>
  <div class="iphone-11-pro-max-copy-12">
    ...
    <div class="userentry-form-empty">
      <p class="form-label3">Bio</p>
      <textarea class="text-box" v-model="selected_input"></textarea>
    </div>
  ...
  methods: {
    onSelectNext() {
      if (this.selected_input == null) {
        alert("Oops! Something went wrong. Please enter your bio.");
      }
      else if (this.selected_input.length > 220) {
        alert("Oops! Something went wrong. Please shorten your bio.");
      }
      else {
        console.log("selected:",this.selected_input);
        //call server to store age in DB
        let fetch_url = '/api/bioupdate?h=' + this.session_hash + '&b=' + this.selected_input;
        ...
        .then(response => {
          ...
          response.json().then(data => {
            if ('error' in data) { //error handling from testroom backend call
              console.log("bio error: ",data['error']);
              alert("bio error: " + data['error']);
              return;
            }
